        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          {trainResult && (
            <div className="space-y-3">
              {trainResult.modelFile && (
                <div className="bg-indigo-950/40 border border-indigo-500/30 rounded-xl p-3 flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-xs">
                  <div className="flex items-center gap-2">
                    <span className="px-2 py-0.5 rounded bg-indigo-500/30 text-indigo-300 font-bold text-[10px] uppercase tracking-wider border border-indigo-400/30">
                      MODEL PERSISTED
                    </span>
                    <span className="text-slate-200 font-medium">Model File Saved:</span>
                    <code className="text-emerald-300 font-mono bg-slate-900 px-2 py-0.5 rounded border border-slate-700/80">
                      {trainResult.modelFile}
                    </code>
                  </div>
                  <span className="text-[11px] text-slate-400 italic">
                    Binary JSON weights & feature metrics written to filesystem
                  </span>
                </div>
              )}
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                <div className="bg-slate-900 border border-slate-800 rounded-xl p-3 space-y-1">
                  ...
                  <div className="text-xl font-bold text-emerald-400">
                    {(trainResult.accuracy <= 1.0 ? trainResult.accuracy * 100 : trainResult.accuracy).toFixed(1)}%
                  </div>
                  <span className="text-[10px] text-slate-500 block">
                    F1 Score: {(trainResult.f1Score <= 1.0 ? trainResult.f1Score * 100 : trainResult.f1Score).toFixed(1)}%
                  </span>
                </div>
